...
export type ContentBlock =
  | { type: 'paragraph'; start: number; end?: number; text?: string; splitAfterSentence?: number; suppressVideos?: boolean }
  ...
  return (
    <section className="px-[60px] pb-16">
      <div className="max-w-[1320px] mx-auto flex flex-col gap-[60px]">
        {blocks.map((block, i) => {
          switch (block.type) {
            // Text only — capped at 640px, 24px paragraph spacing.
            case 'paragraph':
              return (
            ...
            case 'two_images':
              return (
                <div key={i} className="grid grid-cols-2 gap-[40px]">
            ...
            case 'one_image':
              return <ImageFrame key={i} src={block.image} aspect="aspect-[16/9]" />;

            // Single full-width video card.
            case 'video':
              return <VideoCard key={i} videoUrl={block.video} title={block.title} />;

            // Text left, image or video right — both fill their half, 40px gap.
            case 'horizontal':
              return (
                <div key={i} className="grid grid-cols-2 gap-[40px] items-start">
                  <AboutParagraph
                    texts={texts}
                    text={block.text}
                    start={block.start}
